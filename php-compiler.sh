...
install_dependencies() {
    ...
    if [ "${DISTRO}" == "centos7" ]; then
        ...
        check_return_code
        yum check-update
        yum -y install gcc make libc-client-devel libxml2-devel pkgconfig openssl-devel bzip2-devel curl-devel libpng-devel libpng-devel libjpeg-devel libXpm-devel freetype-devel gmp-devel libmcrypt-devel mariadb-devel aspell-devel recode-devel httpd-devel postgresql-devel libxslt-devel libwebp-devel libvpx-devel libicu
        check_return_code
    fi
}
